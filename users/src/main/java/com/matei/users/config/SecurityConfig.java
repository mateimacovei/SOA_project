...
public class SecurityConfig {
    ...
    @Bean
    public SecurityFilterChain filterChain(HttpSecurity http, UserDetailsProvider userDetailsProvider, JwtService jwtService)
            throws Exception {
...
//                        .hasAnyRole(Role.USER.name(), Role.ADMIN.name()   )
    ...
    public WebClient jwtFaasWebClient(@Value("${soa.project.properties.azure.faas}") String faasUrl) {
        return WebClient.builder().baseUrl(faasUrl).build();
    }
